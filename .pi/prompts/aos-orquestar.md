...
6. Reservar archivos compartidos al orquestador (plan maestro, changelog, registry externo si existe); los workers devuelven recomendaciones, no editan esos archivos.
...
Patron probado: para `align all` multi-repo, usar un worker por repo, reservar el registry externo al orquestador, pedir a cada worker una recomendacion de registry y consolidar al final con `context:index` + `context:audit`.
