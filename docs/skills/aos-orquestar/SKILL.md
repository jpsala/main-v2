...
Archivos compartidos reservados al orquestador: <plan/changelog/registry externo si existe/etc.; no editarlos>.
...
- No permitir que workers editen registries externos/planes compartidos; deben devolver recomendaciones para que integre el orquestador.
...
`align all` multi-repo funciono con un worker por repo, workers sin permiso para editar el registry externo, recomendaciones de registry como output y consolidacion final en el hilo principal. Reusar este patron para migraciones/auditorias por lotes.
