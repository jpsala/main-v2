...
GetMainSeqAOptions() {
    return {
        ...
        items: [
            ...
            { key: "i", label: "Pi launcher", idleTimeoutSeconds: 5, items: [
                { key: "p", label: "Plan compacto", doc: "Sol medium + plan-mode local; pensar y aprobar antes de editar.", command: "pi-menu.ps1 -Preset plan", action: () => RunPiMenuPreset("plan") },
                { key: "a", label: "Arquitectura / decision", doc: "Sol high; ambiguedad, tradeoffs y trabajo de alto valor.", command: "pi-menu.ps1 -Preset architecture", action: () => RunPiMenuPreset("architecture") },
                { key: "c", label: "Implementar normal", doc: "Terra medium; codigo cotidiano con tools y contexto mixto.", command: "pi-menu.ps1 -Preset code", action: () => RunPiMenuPreset("code") },
                { key: "b", label: "Ejecutar tarea cerrada", doc: "Luna max; contrato claro, checks definidos y bajo costo por tarea.", command: "pi-menu.ps1 -Preset bounded", action: () => RunPiMenuPreset("bounded") },
                { key: "m", label: "Cambio mecanico", doc: "Luna high; cambio repetible de uno o dos archivos.", command: "pi-menu.ps1 -Preset mechanical", action: () => RunPiMenuPreset("mechanical") },
                { key: "r", label: "Revisar sin editar", doc: "Sol high; juicio independiente y herramientas read-only.", command: "pi-menu.ps1 -Preset review", action: () => RunPiMenuPreset("review") },
                { key: "q", label: "Consulta clara", doc: "Luna medium; contexto minimo y sin extensiones costosas.", command: "pi-menu.ps1 -Preset quick", action: () => RunPiMenuPreset("quick") },
                { key: "d", label: "Delegar investigacion", doc: "Sol medium; subagentes acotados bajo pedido.", command: "pi-menu.ps1 -Preset delegate", action: () => RunPiMenuPreset("delegate") },
                { key: "o", label: "Orquestar trabajo", doc: "Sol medium + Taskflow bajo pedido.", command: "pi-menu.ps1 -Preset orchestrate", action: () => RunPiMenuPreset("orchestrate") },
                { key: "s", label: "AOS normal", doc: "Sol medium + core AOS, advisor, Lens, busqueda y browser.", command: "pi-menu.ps1 -Preset aos", action: () => RunPiMenuPreset("aos") },
                { key: "x", label: "Complex (alias Arquitectura)", doc: "Alias compatible de architecture.", command: "pi-menu.ps1 -Preset complex", action: () => RunPiMenuPreset("complex") },
                { key: "h", label: "Ayuda", doc: "Explica los presets y la seleccion de sesion sin ejecutar Pi.", command: "ShowPiMenuHelp()", action: () => ShowPiMenuHelp() },
            ] },
            { key: "m", label: "psmux main", doc: "Opens the main psmux session in a new Windows Terminal window with a draggable title bar and no outer tab row while only one tab is open.", command: "wt.exe -w new -p psmux", action: () => OpenPsmuxTerminal() },
            ...
            { key: "z", label: "WezTerm themes", idleTimeoutSeconds: 5, items: [
                ...
                { key: "k", label: "Kanagawa / Fox", items: [
                    ...
                    { key: "w", label: "Kanagawa Wave", action: () => OpenWezTermTheme("Kanagawa Wave", "kanagawa-wave.lua", "Kanagawa (Gogh)") },
                ...
                { key: "o", label: "Other dark", items: [
                    ...
                    { key: "r", label: "Rose Pine Moon", action: () => OpenWezTermTheme("Rose Pine Moon", "rose-pine-moon.lua", "rose-pine-moon") },
                ] },
            ] },
